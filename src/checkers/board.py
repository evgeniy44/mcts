...
class Board:
	...
	def copy(self):
		...
		board.position_count = self.position_count
		board.rows_per_user_with_pieces = self.rows_per_user_with_pieces
		board.position_layout = self.position_layout
		board.piece_requiring_further_capture_moves = None
		board.previous_move_was_capture = self.previous_move_was_capture
		board.pieces = []
		for piece in self.pieces:
			board.pieces.append(piece.copy(board))
		board.searcher = BoardSearcher()
		board.searcher.build(board)
		return board
	...
	def create_new_board_from_move(self, move):
		...
		if move in new_board.get_possible_capture_moves():
